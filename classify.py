...
while(cap.isOpened()):
    ...
    if ret==True:
        ...
        cv2.imwrite('./examples/'+str(name)+'.jpg',frame)
        name=name+1
...
for i in range(2,6,1):
	print(i)
	image = cv2.imread("./examples/"+files[i])
	output = image.copy()
	print(files)
	...
	if(danger==3):
		...
		image = client.upload_from_path('examples/3.jpg', config=config, anon=False)
		print("Done")
